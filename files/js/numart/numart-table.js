    ...
    td.addEventListener('dblclick', function(e) {
        ...
        if (column.type === 'select') {
            ...
            const saveEdit = () => {
                ...
                td.innerHTML = originalContent;
                if (column.key === 'nombre') {
                    td.innerHTML = `<a href="mirar/?id=${product.id}" class="cell-nombre text-link">${newValue}</a>`;
                } else {
                    td.textContent = newValue;
                }
...
// Acciones
function addColumn(columnsArray = []) {
    ...
    columnsArray.forEach(col => {
        ...
        // Agregar propiedad a todos los productos existentes
...
function addRow(data = {}) {
    ...
    // Añadir datos por defecto o los que envíes
    state.columns.forEach(col => {
        newProduct[col.key] = data[col.key] !== undefined 
            ? data[col.key] 
            : col.key === 'tipo' ? 'Producto' :
              col.key === 'precio' ? 0 :
